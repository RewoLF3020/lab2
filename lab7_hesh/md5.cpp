...
std::vector<unsigned char> hashToArr()
{
    ...
    memcpy(&hash[0], &A, 4);             //Функция memcpy копирует num байтов первого блока памяти,
    memcpy(&hash[4], &B, 4);             //на который ссылается указатель srcptr, во второй блок памяти, на который ссылается указатель destptr.
    memcpy(&hash[8], &C, 4);
    memcpy(&hash[12], &D, 4);
    return hash;
}
...
{
    std::string src;
    std::cout << "Input string; enter 'q' to quit:\n";
    while (1)
    {
        std::cin >> src;
        if (src == "q")
            break;
        std::string hash = MD5(src);
